...
export async function GET(req: Request) {
  try {
    ...
    const upstream = await fetch(_joinUrl(mainApiUrl, "/v3/contacts"), {
      method: "GET",
      headers: {
        Authorization: auth,
    ...
    console.error("Error in contacts API route (GET):", e)
...
export async function PUT(req: Request) {
  try {
    ...
    console.error("Error in contacts API route (PUT):", e)
    return NextResponse.json({ error: "Internal server error" }, { status: 500 })
  }
}
